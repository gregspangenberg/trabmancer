...
for i, bone_files in enumerate(grouped_files):
    ...
    # hvcpc.color("grey7", alpha=0.8)

    ost.offest_neckshaft(-20)
    plane_n = ost.plane
    print(ost.neckshaft_rel)
    hvn1 = hv.clone()
    hvn2 = hv.clone()
    ...
    hvcnc.color("black", alpha=0.55)
    # hvcnc.color("grey7", alpha=0.8)
    ...
    plt.screenshot("img/height+_ns-.png").close()
    ...
    plt.add_ambient_occlusion(radius)
    plt.show(*hvs, size=(1200, 1800), zoom=1.8)
    plt.screenshot("img/height+_ns+.png")
    plt.close()
    break
